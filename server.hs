...
main = do
    ...
    execParser (opts homef) >>= \case
        ...
        Options{..} -> doAction optTimeout optJisyo optPort optCode
  where
    ...
    options homef = Options
        ...
        <*> option    (defOpts "timeout" <> value 200000 <> help "google ime request timeout" <> showDefault)

doAction :: Int -> FilePath -> Word16 -> String -> IO ()
doAction to jisyo usePort charCode =
    ...
    serve HostAny (show usePort) $ \(sock, _) -> do
        ...
        forever ( do
            ...
            maybe (throwIO End) (talk to icu mgr conn sock . toKey icu) mbmsg
...
talk :: Int -> ICU.Converter -> Manager -> SQLite.Connection -> Socket -> T.Text -> IO ()
talk to icu mgr conn sock key
    | T.head key == '0' = throwIO End
    | T.head key == '1' = lookupCandidates to False icu conn (T.tail key) mgr >>= send sock
    | T.head key == '2' = send sock "0.1 \n"
    | T.head key == '3' = send sock "127.0.0.1:0.0.0.0: \n"
    | T.head key == '4' = lookupCandidates to True icu conn (T.tail key) mgr >>= send sock
    | otherwise         = return ()
...
lookupCandidates :: Int -> Bool -> ICU.Converter -> Connection -> T.Text -> Manager -> IO S.ByteString
lookupCandidates to cand icu conn k mgr = SQLite.withTransaction conn $ do
    i <- fromOnly . head <$> SQLite.query conn "SELECT count(*) FROM googleime WHERE key = ? LIMIT 1" (Only k)
    if (i :: Int) > 0
        then return ()
        else timeout to () (requestGoogleIme k mgr >>= 
            mapM_ (\r -> do
